...
/** ShaderBeing v2 — مكتوب ضد Skia 1.5.0 المثبتة فعليًا:
 *  source = SkRuntimeEffect عبر Skia.RuntimeEffect.Make + حارس null + زمن بـ tick عام.
 *  إن فشل تجميع الشيدر على أي GPU ⇒ سقوط تلقائي إلى DigitalBeing (لا كراش أبدًا). */
...
export default function ShaderBeing({ presence, size = 280, awaken = 1 }: { presence: PresenceState | null; size?: number; awaken?: number }) {
  const p = presence;
  const effect = useMemo(() => { try { return Skia.RuntimeEffect.Make(SOURCE); } catch { return null; } }, []);
  const [time, setTime] = useState(0);
  useEffect(() => {
    if (!effect) return;
    const iv = setInterval(() => setTime((t) => t + 0.033), 33);
    return () => clearInterval(iv);
  }, [effect]);
  if (!effect) {
    return <DigitalBeing presence={(p as any) ?? null} size={size} isDark maturity={0.85} awaken={awaken} />;
  }
  const uniforms = {
    size: [size, size],
    time: sf(time),
    energy: sf(p?.energy ?? 0.55),
    turbulence: sf(p?.turbulence ?? 0.18),
    voice: sf(p?.voiceLevel ?? 0),
    anticipation: sf(p?.anticipation ?? 0.25),
    awaken: sf(awaken),
    eyeOpen: sf(p?.eyeOpenness ?? 0.85),
    gazeX: sf(p?.gazeX ?? 0),
    gazeY: sf(p?.gazeY ?? 0),
    colorA: [sf(p?.colorA?.r ?? 155) / 255, sf(p?.colorA?.g ?? 111) / 255, sf(p?.colorA?.b ?? 255) / 255],
    colorB: [sf(p?.colorB?.r ?? 70) / 255, sf(p?.colorB?.g ?? 139) / 255, sf(p?.colorB?.b ?? 255) / 255],
    eyeColor: [sf(p?.eyeColor?.r ?? 245) / 255, sf(p?.eyeColor?.g ?? 242) / 255, sf(p?.eyeColor?.b ?? 255) / 255],
  };
  return (
    <Canvas style={{ width: size, height: size }}>
      <Rect x={0} y={0} width={size} height={size}>
        <Shader source={effect} uniforms={uniforms as any} />
      </Rect>
    </Canvas>
  );
}

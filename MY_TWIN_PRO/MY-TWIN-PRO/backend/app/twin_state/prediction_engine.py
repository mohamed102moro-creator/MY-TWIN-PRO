"""Prediction Engine v1.1 — تنبؤ + تسجيل PredictionMade لحلقة التعلم."""
import logging
...
class PredictionEngine:
    ...
    async def _predict_mood(self, ctx: Dict) -> str:
        ...
        if not emotional: return "neutral"
        dominant = emotional.get("dominant_emotion", "neutral")
        for p in emotional.get("patterns", []):
            if "evening" in str(p) and "sadness" in str(p): return "sadness"
        return dominant
    async def _suggest_topics(self, ctx: Dict) -> List[str]:
        ...
        if recent:
            ...
            for kw in ["عمل", "دراسة", "رياضة", "عائلة", "سفر", "صحة", "ترفيه", "علاقات"]:
                if kw in text: topics.append(kw)
    ...
    async def _generate_recommendation(self, ctx: Dict) -> str:
        ...
        recs = {"sadness": "غدًا قد يكون يومًا أنسب للراحة والعناية بنفسك. أنا هنا لدعمك.", "joy": "طاقتك الإيجابية ستستمر غدًا! استغلها في شيء تحبه.", "fear": "لا تقلق، مهما كان ما يقلقك، يمكننا مواجهته معًا.", "neutral": "غدًا يوم جديد مليء بالفرص. أنا بانتظارك."}
        return recs.get(dominant, recs["neutral"])
prediction_engine = PredictionEngine()
logger.info("✅ Prediction Engine v1.1 ready (PredictionMade)")
